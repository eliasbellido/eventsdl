...
open class SetIntervalLiveData<in P, R>(
    ...
    //Only run the timer if there's an active observer
     ...
     *
     * This will not execute when there're no observers.
     * */
    @UiThread
    private fun onInterval(){
        if (!isActive) return // don't process if no one is listening
